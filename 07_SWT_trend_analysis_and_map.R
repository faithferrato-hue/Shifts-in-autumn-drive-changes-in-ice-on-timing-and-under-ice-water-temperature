...
autumn_swt_ice_on_plot <- ggplot(swt_fall_avg_ice_on_doy_cor, aes(x = swt_fall_avg, y = ice_on_doy)) +
  ...
  geom_smooth(method = "loess", se = TRUE, color = "blue", fill = "lightblue", linetype = "dashed")+
  stat_cor(
    method = "spearman",
    label.x.npc = "left",
    label.y.npc = "top",
...
# Read in your data (file dowloaded above)
file_path <- "C:\location\sens_slope_results_swt.csv"
...
# Plotting code for 50-year map (to add significance of trend as shape, we added a column call sig with YES or NO)
trend_map_50 <- ggplot(data = world) +
  geom_sf() +
  ...
  ggtitle("Trends in fall surface water temperature") +
