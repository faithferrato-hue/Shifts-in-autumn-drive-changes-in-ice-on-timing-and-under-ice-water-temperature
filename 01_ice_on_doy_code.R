...
# Read in your data
# This was just a summary table of the coordinates, sens slope trends, and significance of trends (y/N)
ice_on_loc50 <- read_csv("C:/location/ice_on_trend_50.csv") 
world50 <- ne_states(country = "Finland", returnclass = "sf")

#50year right map 
ice_on_trend_map <- ggplot(data = world50) +
  geom_sf() +
  geom_point(data = ice_on_loc50 %>% filter(!is.na(x50_year_sens_slope_c_decade), !is.na(longitude), !is.na(latitude)), 
             aes(x = longitude, y = latitude, color = x50_year_sens_slope_c_decade, shape = sig_50), size = 3) +
  scale_color_gradient(low = "blue", high = "red", name = "days/decade", limits = c(-1.0, 13.5)) +
  labs(x = "Longitude", y = "Latitude", color = "days/decade", shape = "Significant", subtitle = "1972-2021") +
  ggtitle("Ice-on day of year trends") +
  theme_bw() +
