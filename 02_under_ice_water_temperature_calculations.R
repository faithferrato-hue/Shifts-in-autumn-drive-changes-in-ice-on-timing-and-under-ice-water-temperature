...
# Plotting code for under-ice water temperature map
UI_temp_map <- ggplot(data = world) +
  geom_sf() +
  geom_point(data = UI_temp_loc_1985_2021 %>% na.omit(), 
             aes(x = long, y = lat, color = trend, shape = sig), size = 3) +
  scale_shape_discrete(name = "Significant", guide = guide_legend(order = 1)) +   # FIRST
  scale_color_gradient(low = "blue", high = "red", name = "°C/decade", limits = c(-0.28, 0.33), 
                       guide = guide_colorbar(order = 2)) +                       # SECOND
  labs(x = "Longitude", y = "Latitude", subtitle = "1985-2021") +
  ggtitle("Under-ice bottom water temperature trends") +
  theme_bw() +
